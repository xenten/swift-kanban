...
class HelixMenu {
	...
	function __construct( &$params,$_menu){
		global $Itemid;
		$this->_params = $params;
	...
	function  loadMenu(){
		...
		$rows = $items->getItems('menutype', $this->getParam('menutype', 'mainmenu'));
	...
	function genMenuItem($item, $level = 0, $pos = '')
	{
		...
		//Menu image
		if ($this->getParam('show_menu_image') && $iParams->get('menu_image') && $iParams->get('menu_image') != -1) {
	...
	function loadSubContent($item=null,$subItem=null,$level=0,$ret=true) {
		$data='';
		if (!$item->megaparams->get('group')) $data .= '<ul class="sp-menu level-' . $level . '"><li class="menu-item group"><div class="group"><div class="group-content">';
	...
	function beginUl($pid=0, $level=0, $ret = false){
		...
		if ($level) {
			if ($this->_menu=='mega') {//For megamenu
				...
				$data .= '<div class="submenu-wrap">';
				$data .= '<div class="menu-tl"><div class="menu-tr"><div class="menu-tm"></div></div></div>';
				$data .= '<div class="menu-l"><div class="menu-r"><div class="submenu-inner clearfix"' . $style . '>';
				}
		...
		if ($level) {
			if ($this->_menu=='mega') {
				if ($this->items[$pid]->megaparams->get('group')) {
					...
					$data .= '</div></div></div>';
					$data .= '<div class="menu-bl"><div class="menu-br"><div class="menu-bm"></div></div></div>';
					$data .= '</div></div>';
				}	
	...
	function beginLi($mitem=null, $level = 0, $pos = ''){
		...
		echo "<li $active>";
		if ($this->_menu=='mega' && $mitem->megaparams->get('group')) echo "<div class=\"group\">";//If megamenu
	}
	...
	function genClass ($mitem, $level, $pos) {
		...
		if ($this->_menu=='mega') {
			if (@$this->children[$mitem->id] || ($mitem->megaparams->get('subcontent')=='modules') || ($mitem->megaparams->get('subcontent')=='positions') ) { 
				...
				elseif ($level < $this->getParam('endlevel')) $cls .= " parent";
		}
			...
			for ($col=0;$col<$cols && $j<$total;$col++) {
				...
				while ($i < $tmp->col[$col] && $j<$total) {
					...
					if ($this->_menu=='mega' && $row->megaparams->get('group')) $this->genMenuItems( $row->id, $level ); //Do not increase level if group
						elseif ($level < $this->getParam('endlevel')) $this->genMenuItems( $row->id, $level+1 );
