...
function NewsfeedsHPBuildRoute(&$query)
{
	static $items;
...
function NewsfeedsHPParseRoute($segments)
{
	$vars	= array();
	...
	if((int) $alias[0] > 0)
	{
		...
		$db =& JFactory::getDBO();
		if($vars['view'] == 'newsfeed')
		{
			$query = 'SELECT id FROM #__newsfeeds WHERE alias = '.$db->Quote($vars['id']);
		} elseif($vars['view'] == 'category') {
			$query = 'SELECT id FROM #__categories WHERE section = \'com_newsfeeds\' && alias = '.$db->Quote($vars['id']);
		}
		$db->setQuery($query);
