        ...
        function getinputfields($id, $fields, $backend, $fontend, $backendcategory, $exist_options, & $body = null, &  $str = null, &  $str_options= null)
        {
            ...
            if(count($fields)>0){
                 ...
                 if(count($fields)>0){
                  ...
                  if($backend){
                    if(!empty($str)) $str .= '</ul><div class="fieldscontent"> ';
                  ...
                  foreach($fields as $field)
                    {
                      //if($field->position == 1){
                            ...
                            if($field->idgroup != $idgroup){
                                if($idgroup > 0) {
                                    if($backend){
                                        ...
                                        {
                                            if($this->exist_options) $str_options .='</ul></fieldset></div></div> ';
                                        }
                                ...
                                if($backend){
                                    ...
                                    }else
                                    {
                                        ...
                                        if(!empty($field->descriptiongroup)) $str_options .= '<div class="desc">'.$field->descriptiongroup.'</div>';
                            ...
                            }else{ $str.= '<h3 class="frontendtitle" id="meta-fieldsattach'.$field->id.'">';}
                            if(($field->position == 1)||($fontend)){ $str .= $field->title."";}
                            if($backend){
                                if($field->position == 1){
                                    $str .= '</label>';
									if($field->required) {$str .= '  <span style="color:#f00;font-size:0.9em;">(* Required)</span>';}
                                    $str .= '</div>';
                                }
                            }else{if($field->required) {$str .= '  <span class="star">&#160;*</span>';} $str.= '</h3>'; }
              ...
              if($backend)
              {
                    ...
                    if(!empty($str_options)){$str_options .='</ul></fieldset></div></div> ';  }
              }else{
                    $str .='</div></fieldset>';
              }
              if($backend){
