...
<form id="searchForm" action="<?php echo JRoute::_('index.php?option=com_fieldsattach');?>" method="post">
	...
	<?php endif; ?>
        ...
        <!--<fieldset class="only">
            <legend><?php echo JText::_('COM_SEARCH_SEARCH_ONLY');?></legend>-->
            <?php foreach ( $this->fields['search'] as $obj) :
                
                    $val = $obj->id;
                    $txt = $obj->title;
                    $groupid = $obj->idgroup;
                    $grouptitle = $obj->titlegroup;
                    $checked = is_array($this->fields['active']) && in_array($val, $this->fields['active']) ? 'checked="checked"' : '';
             
            ?>
            <?php if($groupid_anterior != $groupid ) {
                $groupid_anterior = $groupid;
                $first = false;
                ?>
                <?php if(!$first){?> </fieldset><?php } ?>
                <fieldset class="only">
                <legend><?php echo JText::_($grouptitle); ;?></legend>
                
            <?php } ?>
                    <input type="checkbox" name="fields[]" value="<?php echo $val;?>" id="field-<?php echo $val;?>" <?php echo $checked;?> />
                    <label for="field-<?php echo $val;?>">
                            <?php echo JText::_($txt); ?>
                    </label>
            <?php endforeach; ?>
         </fieldset> 
