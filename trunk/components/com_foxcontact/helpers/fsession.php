...
class FSession
	...
	public function Save($string, $keyword)
		...
		if ((bool)$this->db->getNumRows())
			...
			$sql = "UPDATE #__" . $GLOBALS["ext_name"] . "_sessions SET data = '$string', birth = '" . $now->toMySQL() . "' WHERE id = '$this->Id' AND cid = $this->Cid AND mid = $this->Mid AND keyword = '$keyword';";
			$this->db->setQuery($sql);
			$result = $this->db->query();
			}
		else
			...
			$sql = "INSERT INTO #__" . $GLOBALS["ext_name"] . "_sessions (id, cid, mid, keyword, birth, data) VALUES ('$this->Id', $this->Cid, $this->Mid, '$keyword', '" . $now->toMySQL() . "', '$string');";
			$this->db->setQuery($sql);
			$result = $this->db->query();
			}
